...
export default class Model {
  ...
  compareTopTile(index, rowIndex, colIndex, clr) {
    ...
    // 非最顶部
      ...
      this.grid[topIndex] !== undefined &&
      this.grid[topIndex].clr === clr
    ) {
      return topIndex
    }
    // 颜色不匹配
  ...
  compareRightTile(index, rowIndex, colIndex, clr) {
    ...
    // 非最右部
      ...
      this.grid[rightIndex] !== undefined &&
      this.grid[rightIndex].clr === clr
    ) {
      return rightIndex
    }
    // 颜色不匹配
  ...
  compareBottomTile(index, rowIndex, colIndex, clr) {
    ...
    // 非最底部
      ...
      this.grid[bottomIndex] !== undefined &&
      this.grid[bottomIndex].clr === clr
    ) {
      return bottomIndex
    }
    // 颜色不匹配
  ...
  compareLeftTile(index, rowIndex, colIndex, clr) {
    ...
    // 非最左部
      ...
      this.grid[leftIndex] !== undefined &&
      this.grid[leftIndex].clr === clr
    ) {
      return leftIndex
    }
    // 颜色不匹配
  ...
  hasNeighbour(index, rowIndex, colIndex) {
    ...
    let clr = tile.clr
    if (this.compareTopTile(index, rowIndex, colIndex, clr) !== false)
      return true
    else if (this.compareRightTile(index, rowIndex, colIndex, clr) !== false)
      return true
    else if (this.compareBottomTile(index, rowIndex, colIndex, clr) !== false)
      return true
    else if (this.compareLeftTile(index, rowIndex, colIndex, clr) !== false)
      return true
    else return false
  }
  // 寻找相邻同色砖块
    ...
    // 周边有相同颜色砖块
      ...
      while (true) {
        ...
        sameClrTiles.forEach(index => {
          ...
          let colIndex = index % this.col,
            rowIndex = (index / this.col) >> 0
          nextSameClrTiles.push(
            ...this.searchSameClr(index, rowIndex, colIndex, clr)
          )
          this.deleteCell(index, rowIndex, colIndex, count) && ++count
        })
        // 下一次的同色砖块数量为0，表示清除结束
        ...
        if (bottomPit - topPit + 1 === pitCount) {
          ...
          if (topPit > start) {
            ...
            for (
              let i = fromIndex;
              i >= toIndex;
              i -= this.col, bottomPitIndex -= this.col
            ) {
              // 执行压缩
