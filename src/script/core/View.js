...
export default class view {
  constructor(view) {
    ...
    this.area.on('ontouchstart' in document ? 'tap' : 'click', e => {
      ...
      let x = e.data.global.x,
        y = e.data.global.y - this.area.y
      let col = (x / this.gridWidth) >> 0,
        row = (y / this.gridHeight) >> 0
    ...
    PIXI.loader
      ...
      .load(() => {
        ;['red', 'green', 'blue', 'yellow', 'purple'].forEach((clr, index) => {
          this.tileTextures[index].baseTexture =
            PIXI.utils.TextureCache[clr].baseTexture
          this.starTextures[index].baseTexture =
            PIXI.utils.TextureCache['star_' + clr].baseTexture
        })
        // 暂停与播放按钮
        ...
        this.stage.addChildAt(
          new PIXI.Sprite(PIXI.utils.TextureCache['cover']),
          0
        )
      })
  }
  ...
  init() {
    ...
    this.showLevelInfo().then(() => {
      ...
      for (let len = this.tiles.length, i = len - 1; i >= 0; --i) {
        ...
        TweenMax.from(tile.sprite, 0.5, {
          y: '-=' + ((600 - Math.random() * this.gridHeight) >> 0),
          delay: (((len - i) / this.col) >> 0) * 0.05,
  ...
  showLevelInfo() {
    ...
    return new Promise((resolve, reject) => {
      ...
      TweenMax.to(this.levelInfo, 0.6, {
        x: -this.width,
        delay: 3,
        onComplete: () => {
  ...
  generateTileTextures() {
    this.tileTextures = this.originClrs.map(clr => {
  ...
  generateStarTextures() {
    console.log(PIXI)
    console.log(this.originClrs)
    this.starTextures = this.originClrs.map(clr => {
      ...
      // 没砖块了
        ...
        // 游戏结束
          ...
          if (this.bounty > 0) {
            ...
            TweenMax.to(this.bountyLabel, 0.6, {
              top: 80,
              onComplete: () => {
  ...
  // 显示色砖分数
  showTileScore(tile) {
    let score = new PIXI.Text(tile.score, {
      fontFamily: 'Arial',
      fontSize: 32,
      fill: 0xffffff,
      align: 'center'
